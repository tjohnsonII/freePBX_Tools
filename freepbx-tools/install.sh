...
check_after_installs() {
  ...
  for c in python3 jq dot mysql; do
    if [[ "$c" == "dot" ]]; then have dot || { warn "Missing dependency: dot (graphviz)"; ((missing++)); }; continue
  fi
  if [[ "$c" == "mysql" ]]; then have mysql || { warn "Missing dependency: mysql client"; ((missing++)); }; continue
  fi
...
check_after_installs() {
  ...
  for c in python3 jq dot mysql; do
    if [[ "$c" == "dot" ]]; then
      have dot || { warn "Missing dependency: dot (graphviz)"; ((missing++)); }; continue
    fi
    if [[ "$c" == "mysql" ]]; then
      have mysql || { warn "Missing dependency: mysql client"; ((missing++)); }; continue
    fi
    have "$c" || { warn "Missing dependency: $c"; ((missing++)); }
  done
...
main "$@"
is_el7() { [[ -f /etc/redhat-release ]] && grep -qE 'release 7\.' /etc/redhat-release; }
enable_epel_if_needed
is_el() { [[ -f /etc/redhat-release ]]; }
is_el7(){ is_el && grep -qE 'release 7\.' /etc/redhat-release; }
is_el8(){ is_el && grep -qE 'release 8\.' /etc/redhat-release; }
...
ls -l "$BINDIR"/freepbx-* "$BINDIR"/asterisk-full-diagnostic 2>/dev/null || true
blu "Output directory: $CALLFLOWS_DIR (owned by asterisk)"

#!/usr/bin/env bash
# 123NET FreePBX Tools - Installer
# - Installs to /usr/local/123net/freepbx-tools
# - Symlinks commands into /usr/local/bin
# - Ensures deps (jq, graphviz/dot, mysql client, python3) with EL7 python36u safety
# - Creates /home/123net/callflows (owned by asterisk if present)
# - Prints version policy banner using version_check.sh/py when available
