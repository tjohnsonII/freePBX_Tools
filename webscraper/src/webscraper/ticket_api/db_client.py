...
def upsert_discovered_handles(db_path: str, rows: list[dict[str, Any]]) -> int:  # noqa: ARG001
    if not rows:
        return 0
    r = _post("/api/ingest/handles", {"rows": rows})
...
def upsert_tickets_batch(
    ...
    if not rows:
        return 0
    r = _post("/api/ingest/tickets", {"handle": handle, "tickets": rows})
    return int(r.get("inserted", 0))
...
def upsert_noc_queue_tickets(
    ...
    if not records:
        return 0
    r = _post("/api/ingest/noc-queue", {"records": records, "now_utc": now_utc})
    return int(r.get("inserted", 0))
...
def upsert_vpbx_records(
    ...
    if not records:
        return 0
    r = _post("/api/ingest/vpbx/records", {"records": records, "now_utc": now_utc})
    return int(r.get("inserted", 0))
...
def upsert_vpbx_device_configs(
    ...
    if not records:
        return 0
    r = _post("/api/ingest/vpbx/device-configs", {"records": records, "now_utc": now_utc})
    return int(r.get("inserted", 0))
...
def upsert_vpbx_site_configs(
    ...
    if not records:
        return 0
    r = _post("/api/ingest/vpbx/site-configs", {"records": records, "now_utc": now_utc})
    return int(r.get("inserted", 0))
