...
ALIASES_PATH = os.environ.get(
    'TRACEROUTE_ALIASES',
    os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ip_aliases.json')
)
...
def _load_aliases_if_needed():
    global _IP_ALIASES, _ALIASES_MTIME
    try:
        st = os.stat(ALIASES_PATH)
        if st.st_mtime == _ALIASES_MTIME:
            return
        with open(ALIASES_PATH, 'rb') as f:
            _IP_ALIASES = json.loads(f.read().decode('utf-8'))
        _ALIASES_MTIME = st.st_mtime
        logging.info(
            "Loaded IP aliases from %s (%d entries)",
            ALIASES_PATH, len(_IP_ALIASES)
        )
    except Exception as e:
        _IP_ALIASES = {}
        _ALIASES_MTIME = 0
        logging.warning(
            "IP aliases not loaded (%s): %s",
            ALIASES_PATH, str(e)
        )


def _label_for_ip(ip):
    try:
        return _IP_ALIASES.get(ip)
    except Exception:
        return None
...
class TracerouteHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        for raw in decoded:
            ...
            # pure timeout line like: "3  * * *"
                pass
            else:
                # Example: "1 192.168.1.1 0.123 ms 0.456 ms 0.789 ms"
                m1 = re.match(r'^\s*(\d+)\s+.*?([0-9]{1,3}(?:\.[0-9]{1,3}){3}).*?([0-9.]+)\s*ms', line)
                if m1:
                    ip = m1.group(2)
